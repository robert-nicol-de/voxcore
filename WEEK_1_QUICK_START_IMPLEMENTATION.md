# Week 1 Quick Start: Implement SQL Validator in 3 Hours

**Goal**: Add syntactic & semantic validation layer  
**Time**: 3-5 hours  
**Accuracy Gain**: +5-10%  
**Complexity**: Medium  
...
## STEP 1: Add sqlglot to Requirements (5 minutes)
...
## STEP 2: Create SQL Validator (45 minutes)
...
## STEP 3: Integrate into Query Endpoint (30 minutes)

**File**: `voxcore/voxquery/voxquery/api/v1/query.py`

Find the `execute_query` function and add validation:
...
## STEP 4: Create Test Cases (30 minutes)
...
class TestMetadataExtraction:
    ...
    def test_extract_tables(self, validator):
        ...
        sql = "SELECT * FROM Sales.Customer JOIN Sales.SalesOrderHeader ON ..."
        is_valid, error, metadata = validator.validate(sql, "test")
        # Will fail due to incomplete JOIN, but we can check tables were extracted
        assert "Customer" in metadata["tables"] or "SalesOrderHeader" in metadata["tables"]
...
- [ ] sql_validator.py created (150 lines)
- [ ] test_sql_validator.py created (100 lines)
- [ ] query.py modified (add 10 lines)
- [ ] All tests pass
- [ ] Backend restarted
- [ ] UI tests pass
...
1. **Week 2**: Implement rewrite engine (fix dialect issues)
2. **Week 3**: Implement semantic router (reduce schema noise)
3. **Week 4**: Implement feedback loop (learn from corrections)
...
```
Solution: Adjust forbidden_tables list or risk_score thresholds
```
...
This Week 1 implementation adds a validation layer that catches:
- ✅ Broken SQL (syntax errors)
- ✅ Forbidden tables (hallucinations)
- ✅ Missing aggregation (incomplete queries)
