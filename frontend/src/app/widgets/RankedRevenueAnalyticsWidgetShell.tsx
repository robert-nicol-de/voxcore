...
import type { RankedRevenueAggregateViewModel } from "./rankedRevenueViewModels";

type SortKey = "dimension" | "revenue" | "share_of_total";

type RankedRevenueAnalyticsWidgetShellProps<TResponse> = {
  aggregatePath: string;
  mapAggregateResponse: (response: TResponse) => RankedRevenueAggregateViewModel;
  chartPalette: string[];
  rowTestIdPrefix: string;
  dimensionColumnLabel: string;
  renderDrilldownModal: (selectedDimension: string, onClose: () => void) => ReactNode;
};

export default function RankedRevenueAnalyticsWidgetShell<TResponse>({
  aggregatePath,
...
}: RankedRevenueAnalyticsWidgetShellProps<TResponse>) {
  const [payload, setPayload] = useState<RankedRevenueAggregateViewModel | null>(null);
  ...
  if (error) {
    return (
      <div className="rounded-[1.5rem] border border-rose-500/30 bg-[#161122] p-6 text-rose-100 shadow-[0_20px_60px_rgba(59,7,18,0.35)]">
        <div className="text-lg font-semibold">{payload?.title || "Governed Revenue Analytics"}</div>
  ...
  return (
    <>
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-3">
              <SummaryStat
                ...
                value={formatCurrency(payload.summary_card.metric_value)}
                tone={payload.summaryTones[0]}
              />
              <SummaryStat
                label={payload.topDimensionLabel}
                value={payload.summary_card.top_dimension}
                helper={formatCurrency(payload.summary_card.top_dimension_revenue)}
                tone={payload.summaryTones[1]}
              />
              <SummaryStat
                label={payload.coverageLabel}
                value={String(payload.summary_card.dimension_count)}
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={payload.data} margin={{ top: 8, right: 16, left: 0, bottom: 16 }}>
                    ...
                    />
                    <Bar dataKey="revenue" radius={[16, 16, 6, 6]}>
                      {payload.data.map((entry, index) => (
                        <Cell
                          key={entry.dimension}
                          fill={chartPalette[index % chartPalette.length]}
                          onClick={() => setSelectedDimension(entry.dimension)}
              ...
              <div className="overflow-hidden rounded-2xl border border-white/6">
                <table className="min-w-full divide-y divide-white/6 text-sm">
                  <thead className="bg-white/5 text-slate-300">
                    <tr>
                      <HeaderButton label={dimensionColumnLabel} onClick={() => toggleSort("dimension")} />
                      <HeaderButton label="Revenue" onClick={() => toggleSort("revenue")} />
                      <HeaderButton label="Share %" onClick={() => toggleSort("share_of_total")} />
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/6 bg-[#0a101c]">
                    {sortedRows.map((row) => (
                      <tr
                        ...
                        onClick={() => setSelectedDimension(row.dimension)}
                        data-testid={`${rowTestIdPrefix}-${row.dimension}`}
                      >
                        ...
                        <td className="px-4 py-3 text-slate-300">{row.share_of_total.toFixed(2)}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {selectedDimension ? renderDrilldownModal(selectedDimension, () => setSelectedDimension(null)) : null}
    </>
  );
}
